...
//					target.__tapout=true;
	...
	// detect OS version
	var toVersion = function(ver) {
		return {
			major: Number(ver[0]) || 0,
			minor: Number(ver[1]) || 0,
			revision: Number(ver[2]) || 0
		};
	}
	if (os.ios) {
		ua.match(/OS\s+([0-9_]+)/);
		os.version = toVersion(RegExp.$1.split('_'));
	} else if (os.android) {
		ua.match(/Android\s+([0-9\.]+)/);
		os.version = toVersion(RegExp.$1.split('.'));
	}
	$.os = os;
	...
	$.ready = function(cb) {
		...
		if (readyState === 'complete' || readyState === 'loaded' || readyState === 'interactive') cb();
	...
	$.viewport = function() {
		return {
			...
			orientation: $.os.mobile ?
				(w.innerHeight > w.innerWidth ?  'portrait' : 'landscape')
				: 'portrait'
		};
	};
